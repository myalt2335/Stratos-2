use crate::{console, time, serial, wait};
use alloc::format;
...
pub fn help() {
    ...
    console::write_line("  time (help)   - Show the current time (from RTC)");
    console::write_line("  thud on|off   - Toggle terminal HUD");
...
pub fn version() {
    console::write_line("StratOS");
    console::write_line("Shell 1.A009.02.251031.EXENUS@cfc8a");
...
pub fn handle_command(input: &str) {
    ...
    match command.as_str() {
        ...
        "thud" => {
            if parts.len() == 2 {
                match parts[1] {
                    "on" => { crate::thud::enable(); console::write_line("Terminal HUD enabled."); console::write_line("Thud is in early stages, please expect errors."); }
                    "off" => { crate::thud::disable(); console::write_line("Terminal HUD disabled."); }
                    _ => console::write_line("Usage: thud on|off"),
                }
            } else {
                console::write_line("Usage: thud on|off");
            }
        }

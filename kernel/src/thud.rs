...
pub fn poll_draw() {
    ...
    with_console(|c| {
        c.hud_begin();
        c.hud_draw_text(left_buf.as_str(), 0xAAAAAA, HudAlign::Left);
        c.hud_draw_text(center_buf.as_str(), 0xAAAAAA, HudAlign::Center);
        c.hud_draw_text(right_buf.as_str(), 0xAAAAAA, HudAlign::Right);
        c.hud_present();
    });
}
